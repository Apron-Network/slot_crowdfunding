...
class State < ApplicationRecord
  ...
  class << self
    ...
    def current_block_num_v2(chain:)
      find_or_create_by! chain: chain.to_s.downcase, key: "current_block_num_v2" do |record|
        record.integer_value = 0
      end
    end

    def current_block_num_v3(chain:)
      find_or_create_by! chain: chain.to_s.downcase, key: "current_block_num_v3" do |record|
        record.integer_value = 0
      end
    end
    ...
    def start_block_num_v2(chain:)
      find_or_create_by! chain: chain, key: "start_block_num_v2" do |record|
        record.integer_value = 1
      end
    end

    def start_block_num_v3(chain:)
      find_or_create_by! chain: chain, key: "start_block_num_v3" do |record|
        record.integer_value = 1
      end
    end

    # https://www.stakingrewards.com/earn/kusama
    SUPPORT_SYMBOLS = %w[APRON DOT KSM].map(&:downcase)
