...
start_block_num_v3 = State.start_block_num_v3(chain: current_chain)
current_block_num_v3 = State.current_block_num_v3(chain: current_chain)

if current_block_num_v3.integer_value < start_block_num_v3.integer_value
  current_block_num_v3.update_attribute :integer_value, start_block_num_v3.integer_value
end

loop do

  # if current_block_num_v3.integer_value > 8_499_000
  #   pp "sync block done，blokc num is ", current_block_num_v3.integer_value
  #   break
  # end

  puts "Fetching chain `#{current_chain}` contribution events at block num #{current_block_num_v3.integer_value}"
  out = URI.open("http://127.0.0.1:3001/fetch_block/#{current_block_num_v3.integer_value}").read rescue nil
  if out.blank? || out == "{}"
  ...
  fund_infos =
    if current_block_num_v3.integer_value % 10 == 0
      o = URI.open("http://127.0.0.1:3001/fund_infos?block_number=#{current_block_num_v3.integer_value}").read rescue nil
  ...
  contribution_events = jsonify_out.fetch("contributions").map do |item|
    {
      chain: current_chain,
      block_num: current_block_num_v3.integer_value,
      who: item.fetch("who"),
      fund_index: item.fetch("fund_index"),
      amount: item.fetch("amount").to_s.to_d,
      on_chain_hash: current_block_num_v3.integer_value,
      timestamp: timestamp,
  ...
  ApplicationRecord.transaction do
    ...
    Campaign
      .where("start_block <= ? AND end_block >= ?", current_block_num_v3.integer_value, current_block_num_v3.integer_value)
      .each do |campaign|
    ...
    current_block_num_v3.update_attribute :integer_value, current_block_num_v3.integer_value + 1
  end
